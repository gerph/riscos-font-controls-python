"""
Structures and classes we use for graphics.

Both Scale and Transform have `transform` and `scale` properties, which give the equivalent
object for that type, or None if the structure cannot be converted.
"""
...
class Bounds(object):
    ...
    def __init__(self, ro, structure=None, x0=1, y0=1, x1=0, y1=0):
        self.ro = ro
        if structure:
            if isinstance(structure, tuple):
                ...
                if isinstance(structure, (int, long)):
                    structure = ro.memory[structure]
                self.x0 = structure.read_signedword(0)
                self.y0 = structure.read_signedword(4)
                self.x1 = structure.read_signedword(8)
                self.y1 = structure.read_signedword(12)
        else:
            self.x0 = x0
            self.y0 = y0
            self.x1 = x1
    ...
    def write_bounds(self, mem, offset=0):
        mem.write_signedword(int(self.x0), offset=offset + 0)
        mem.write_signedword(int(self.y0), offset=offset + 4)
        mem.write_signedword(int(self.x1), offset=offset + 8)
        mem.write_signedword(int(self.y1), offset=offset + 12)
    ...
    def copy(self):
        return self.__class__(self.ro, x0=self.x0, y0=self.y0, x1=self.x1, y1=self.y1)
...
class Transform(object):
    ...
    * 6-element matrix, using 16.16 fixed point values.
    ...
    (dx, dy) = transform.apply(sx, dy)
        - transform a single coordinate pair

    (bl, br, tl, tr) = transform.quad(x0, y0, y1, y1)
        - transform the 4 corners of the supplied box as tuples of coordinate pairs

    (x0, y0, y1, y1) = transform.bbox(x0, y0, y1, y1)
    ...
    def write_matrix(self, mem):
        matrix = self.matrix
        if not matrix:
            raise NotImplementedError("{}.write_matrix cannot write {}".format(self.__class__.__name__, self))
        mem.write_signedword(int(matrix.a * (1<<16)), offset=0)
        mem.write_signedword(int(matrix.b * (1<<16)), offset=4)
        mem.write_signedword(int(matrix.c * (1<<16)), offset=8)
        mem.write_signedword(int(matrix.d * (1<<16)), offset=12)
        mem.write_signedword(int(matrix.e), offset=16)
        mem.write_signedword(int(matrix.f), offset=20)

    def write_scale(self, mem):
        scale = self.scale
        if not scale:
            raise NotImplementedError("{}.write_scale cannot write {}".format(self.__class__.__name__, self))
        mem.write_signedword(int(scale.xmult), offset=0)
        mem.write_signedword(int(scale.ymult), offset=4)
        mem.write_signedword(int(scale.xdiv), offset=8)
        mem.write_signedword(int(scale.ydiv), offset=12)
...
class Matrix(Transform):
    ...
    def __init__(self, ro, structure=None, array=None):
        super(Matrix, self).__init__(ro)
        ...
        if structure:
            if isinstance(structure, (int, long)):
                structure = ro.memory[structure]
            self.a = float(structure.read_signedword(0)) / (1<<16)
            self.b = float(structure.read_signedword(4)) / (1<<16)
            self.c = float(structure.read_signedword(8)) / (1<<16)
            self.d = float(structure.read_signedword(12)) / (1<<16)
            self.e = structure.read_signedword(16)
            self.f = structure.read_signedword(20)
        self.matrix = self
...
class Scale(Transform):
    ...
    def __init__(self, ro, structure=None, array=None):
        ...
        if array:
            ...
            if isinstance(structure, (int, long)):
                structure = ro.memory[structure]
            self.xmult = structure.read_signedword(0)
            self.ymult = structure.read_signedword(4)
            self.xdiv = structure.read_signedword(8)
            self.ydiv = structure.read_signedword(12)
        else:
            self.xmult = 1
            self.ymult = 1
            self.xdiv = 1
